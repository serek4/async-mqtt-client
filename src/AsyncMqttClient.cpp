...
, _onDisconnectUserCallback()
, _onSubscribeUserCallback()
, _onUnsubscribeUserCallback()
, _onMessageUserCallback()
, _onPublishUserCallback()
...
void AsyncMqttClient::_onDisconnect() {
  ...
  _onDisconnectUserCallback(_disconnectReason);
}
...
void AsyncMqttClient::_onConnAck(bool sessionPresent, uint8_t connectReturnCode) {
  ...
  if (connectReturnCode == 0) {
    _state = CONNECTED;
    _onConnectUserCallback(sessionPresent);
  } else {
...
void AsyncMqttClient::_onSubAck(uint16_t packetId, char status) {
  ...
  _onSubscribeUserCallback(packetId, status);
...
void AsyncMqttClient::_onPubAck(uint16_t packetId) {
  ...
  _onPublishUserCallback(packetId);
}
